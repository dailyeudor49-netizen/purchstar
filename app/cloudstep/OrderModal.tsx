...
const PRODUCT_TITLE = "VENOCARE™ PRO";
const PRODUCT_PRICE = 49.90;
const PRODUCT_COMPARE = 99.90;
const SHOP_NAME = "purchstar";

const COLORS = ["Nero", "Grigio Scuro", "Grigio Chiaro"];
const COLOR_HEX: Record<string, string> = { "Nero": "#1a1a1a", "Grigio Scuro": "#4a4a4a", "Grigio Chiaro": "#b0b0b0" };
  ...
  return (
    <>
      ...
      <div className="mb-4" id="taglie">
        <p className="text-sm font-semibold text-gray-500 mb-2.5">Colore</p>
        <div className="flex flex-wrap gap-2.5 mb-4">
          {COLORS.map((c) => (
            <button
              key={c}
              onClick={() => setColor(c)}
              className={`h-[42px] px-4 rounded-full border-2 text-[13px] font-medium transition-all flex items-center gap-2 ${
                color === c
                  ? "border-[#1B5E6B] bg-[#1B5E6B] text-white font-semibold"
                  : "border-gray-300 bg-white text-gray-700 hover:border-gray-500"
              }`}
            >
              <span className="w-4 h-4 rounded-full border border-white/50 shrink-0" style={{ background: COLOR_HEX[c] }} />
              {c}
            </button>
          ))}
        </div>
              ...
              <div className="px-5 pt-4 pb-5">
                ...
                <div className="flex gap-3.5 items-center p-3.5 bg-[#fafafa] border border-gray-200 rounded-xl mb-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[13px] text-gray-400 mb-1">{color} — Taglia {size}</p>
                ...
                <div className="mb-[18px] flex flex-col gap-2.5">
                  <div className="flex gap-3 items-center p-3 bg-[#fafafa] border border-gray-200 rounded-[10px]">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[11px] text-gray-400">{color} — Taglia {size}</p>
                    </div>
