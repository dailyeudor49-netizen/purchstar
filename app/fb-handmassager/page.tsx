...
const REVIEWS = [
  ...
  { id: 2, title: "Un vero aiuto dopo l'ufficio", name: "Valentina S.", rating: 5, comment: "Lavorando al PC sento spesso il collo rigido. 15 minuti con questo dispositivo e mi sento rinata. Molto versatile anche per le cosce.", date: "2 Aprile 2024" },
  { id: 3, title: "Versatile e facilissimo da usare", name: "Roberto L.", rating: 5, comment: "Il calore è molto piacevole. Spedizione rapida e ho pagato comodamente alla consegna. Lo consiglio a chiunque cerchi relax.", date: "22 Aprile 2024" },
  { id: 4, title: "Sensazione di leggerezza diffusa", name: "Alessio P.", rating: 5, comment: "Dopo gli allenamenti sentivo sempre le gambe pesanti. Questo dispositivo aiuta tantissimo a distendere i muscoli rapidamente.", date: "5 Maggio 2024" },
  ...
  { id: 7, title: "Pratico anche in viaggio", name: "Serena B.", rating: 5, comment: "Leggero, lo porto sempre con me. Mi aiuta a mantenere i muscoli rilassati anche durante i lunghi spostamenti di lavoro.", date: "25 Maggio 2024" },
  { id: 8, title: "Potenza regolabile con precisione", name: "Davide V.", rating: 5, comment: "I 15 livelli di intensità sono fantastici. Si può passare da un leggero tocco a una stimolazione muscolare molto profonda.", date: "1 Giugno 2024" },
  ...
  { id: 10, title: "Molto più di un massaggiatore", name: "Matteo S.", rating: 4, comment: "Lo uso anche sulle braccia. Ottima l'idea del pagamento alla consegna, mi ha dato molta sicurezza nell'acquisto online.", date: "15 Giugno 2024" },
  ...
  { id: 12, title: "Leggero ma spinge forte", name: "Stefano C.", rating: 5, comment: "Design a U perfetto per bloccarlo anche sulla zona lombare mentre sei seduto in poltrona. Potente ed efficace.", date: "28 Giugno 2024" },
  { id: 13, title: "Spedizione lampo", name: "Elena F.", rating: 5, comment: "Arrivato in meno di 24 ore. Imballaggio perfetto e prodotto superiore alle aspettative. Il colore bianco è molto elegante.", date: "5 Luglio 2024" },
  { id: 14, title: "Indispensabile per lo smart working", name: "Fabio D.", rating: 5, comment: "Lavorando da casa la postura ne risente. Questo è l'unico modo per finire la giornata senza sentirsi contratti.", date: "12 Luglio 2024" },
  ...
  { id: 16, title: "Top per il benessere quotidiano", name: "Chiara G.", rating: 5, comment: "Lo uso per le gambe la sera. Molto rilassante, lo consiglio a chi sta molto tempo in piedi.", date: "22 Luglio 2024" }
];

// --- COMPONENTI INTERNI ---

// Added explicit React.FC type to AmazonStar to ensure 'key' and other React props are accepted correctly by TypeScript.
...
export default function LandingPage() {
  ...
  return (
    <div className="min-h-screen bg-white text-slate-900 font-sans selection:bg-blue-100">
      ...
      {/* 1. SCARCITY BAR (Neuromarketing: Urgenza) */}
      ...
      {/* 2. HERO SECTION (Massivo Impatto Mobile) */}
        ...
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center gap-16 relative z-10">
          <div className="flex-1 text-center md:text-left">
            ...
            <div className="flex flex-wrap justify-center md:justify-start gap-8 border-t-2 border-slate-100 pt-10 opacity-70">
              <span className="text-xs md:text-sm font-black uppercase tracking-widest">🚚 Consegna Gratuita 24h</span>
      ...
      {/* 3. EMOTIONAL PROBLEM SECTION (FB Compliant: Focus su Tensione e Benessere) */}
      ...
      {/* 4. TIMER SECTION (Neuromarketing: Scarsità Temporale) */}
      <section className="py-20 px-6 bg-white">
        <div className="bg-red-700 py-12 md:py-20 rounded-[4rem] text-center max-w-3xl mx-auto shadow-3xl text-white border-4 border-white/20">
          ...
          <div className="flex justify-center items-center gap-6 md:gap-10 mb-10">
            <div className="flex flex-col"><span className="bg-white text-red-700 text-5xl md:text-8xl font-black p-6 rounded-[2rem] shadow-2xl">{time.m}</span><span className="text-[10px] uppercase font-black mt-4 opacity-60">Minuti</span></div>
            <div className="text-5xl md:text-7xl font-black mb-14">:</div>
            <div className="flex flex-col"><span className="bg-white text-red-700 text-5xl md:text-8xl font-black p-6 rounded-[2rem] shadow-2xl">{time.s}</span><span className="text-[10px] uppercase font-black mt-4 opacity-60">Secondi</span></div>
          </div>
          <p className="text-lg md:text-2xl font-bold italic opacity-90">Completa l'ordine ora per bloccare lo sconto di 100€.</p>
        </div>
      </section>

      {/* 5. REVIEWS SECTION (Amazon Style + Carica Altro) */}
          ...
          <div className="grid lg:grid-cols-3 gap-16">
            ...
            <div className="lg:col-span-2 space-y-16">
              ...
              {visibleReviews < REVIEWS.length && (
                <button onClick={() => setVisibleReviews(prev => prev + 3)} className="w-full py-6 bg-slate-900 text-white font-black rounded-3xl text-xl shadow-xl flex items-center justify-center gap-4 hover:bg-blue-600 transition-all group">
      ...
      {/* 6. ORDER FORM SECTION (High Impact + Offerta Singola) */}
      <section ref={orderRef} className="py-24 md:py-40 px-6">
        ...
        <div className="max-w-2xl mx-auto bg-white rounded-[4rem] shadow-[0_50px_100px_rgba(0,0,0,0.15)] overflow-hidden border border-slate-100 relative">
          ...
          {!submitted ? (
            <form onSubmit={handleOrder} className="p-10 md:p-16 space-y-12">
              <div className="bg-blue-50 p-10 rounded-[3rem] border-4 border-blue-600 relative">
                <span className="absolute -top-4 left-1/2 -translate-x-1/2 bg-blue-600 text-white text-xs font-black px-6 py-2 rounded-full uppercase tracking-widest shadow-xl">OFFERTA DEL GIORNO</span>
              ...
              <div className="space-y-8">
                <input required placeholder="Nome e Cognome" className="w-full p-8 bg-slate-50 border-2 border-slate-50 rounded-3xl focus:border-blue-600 focus:bg-white outline-none text-xl font-bold" />
                <input required placeholder="Cellulare" type="tel" className="w-full p-8 bg-slate-50 border-2 border-slate-50 rounded-3xl focus:border-blue-600 focus:bg-white outline-none text-xl font-bold" />
                <input required placeholder="Indirizzo e N. Civico" className="w-full p-8 bg-slate-50 border-2 border-slate-50 rounded-3xl focus:border-blue-600 focus:bg-white outline-none text-xl font-bold" />
                <input required placeholder="Città e Provincia" className="w-full p-8 bg-slate-50 border-2 border-slate-50 rounded-3xl focus:border-blue-600 focus:bg-white outline-none text-xl font-bold" />
              </div>

              <button disabled={isSubmitting} className="w-full py-10 bg-green-500 hover:bg-green-600 text-white font-black text-3xl uppercase rounded-[3rem] shadow-2xl transition-all hover:scale-[1.05] active:scale-95 flex flex-col items-center justify-center gap-1">
                {isSubmitting ? "ELABORAZIONE..." : <><span>CONFERMA L'ORDINE</span><span className="text-sm opacity-80 font-bold tracking-[0.2em]">PAGA ALLA CONSEGNA</span></>}
              </button>
            </form>
          ) : (
            <div className="p-20 text-center animate-fade-in">
              <div className="w-24 h-24 bg-green-500 rounded-full flex items-center justify-center text-white mx-auto mb-8 text-5xl">✓</div>
              <h3 className="text-4xl font-black mb-6">ORDINE RICEVUTO!</h3>
              <p className="text-xl text-slate-600 italic">Il tuo nuovo percorso di benessere inizia oggi.</p>
            </div>
          )}
        </div>
      </section>
      ...
      <footer className="bg-slate-950 text-slate-500 py-20 px-6 text-center">
        <div className="max-w-4xl mx-auto space-y-10">
          ...
          <p className="text-sm leading-relaxed max-w-xl mx-auto opacity-60">AuraRelief Pro Italia è leader nelle soluzioni per il benessere quotidiano a casa. Non utilizziamo termini medici o promesse di cure. Consultare sempre un esperto per condizioni specifiche.</p>
          <div className="flex justify-center gap-10 text-[10px] font-black uppercase tracking-widest">
            <a href="#">Privacy</a><a href="#">Termini</a><a href="#">Spedizioni</a>
          </div>
          <p className="text-[10px] opacity-40">© 2024 AuraRelief Pro. P.IVA 0123456789 - Tutti i diritti riservati.</p>
        </div>
      </footer>

      {/* FLOATING CTA (Mobile Optimized) */}
      {showFloatingCTA && !submitted && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-[200] w-[95%] max-w-sm">
          <button onClick={scrollToOrder} className="w-full bg-slate-900 text-white py-6 px-10 rounded-[2.5rem] shadow-3xl flex items-center justify-between font-black uppercase text-lg border-2 border-white/20 animate-urgent">
            <span className="flex flex-col items-start leading-none">
              <span className="text-[10px] text-red-400 mb-1 tracking-widest">OFFERTA LAMPO</span>
              <span>ORDINA ORA</span>
            </span>
            <span className="bg-green-500 px-6 py-2 rounded-2xl text-2xl">{PRICE.toFixed(2)}€</span>
          </button>
        </div>
      )}
      ...
      <style jsx global>{`
        ...
        @keyframes urgent { 0% { transform: scale(1) translateX(-50%); } 50% { transform: scale(1.05) translateX(-47.5%); } 100% { transform: scale(1) translateX(-50%); } }
